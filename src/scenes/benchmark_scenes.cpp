...
#include <wbc/solvers/qpswift/QPSwiftSolver.hpp>
...
void evaluateVelocitySceneQuadraticCost(map<string,RobotModelPtr> robot_models, map<string,QPSolverPtr> solvers,
    ...
    for(auto it : robot_models){
        for(auto itt : solvers){
            ScenePtr scene = make_shared<VelocitySceneQP>(it.second, itt.second, 0.001);
...
void evaluateAccelerationSceneTSID(map<string,RobotModelPtr> robot_models, map<string,QPSolverPtr> solvers,
    ...
    for(auto it : robot_models){
        for(auto itt : solvers){
            ScenePtr scene = make_shared<AccelerationSceneTSID>(it.second, itt.second, 0.001);
            if(!scene->configure({cart_task}))
...
void evaluateAccelerationSceneReducedTSID(map<string,RobotModelPtr> robot_models, map<string,QPSolverPtr> solvers,
    ...
    for(auto it : robot_models){
        for(auto itt : solvers){
            ScenePtr scene = make_shared<AccelerationSceneReducedTSID>(it.second, itt.second, 0.001);
            if(!scene->configure({cart_task}))
